...
class EditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val retrofit = Retrofit.Builder()
            .baseUrl("http://192.168.10.26") // Substitua pelo IP ou URL do seu backend
            .addConverterFactory(GsonConverterFactory.create())
            .build()
        apiService = retrofit.create(ApiService::class.java)

        // Pegando dados enviados
        val Id = intent.getIntExtra("tripId", -1) // ID da corrida
        if (Id != -1) {
            // Carregar os dados da corrida do backend
            loadTripDetails(Id, editOriginField, editDestinationField, editDateField, editTimeField)
        }
        ...
        saveChangesButton.setOnClickListener {
            ...
            if (origin.isNotEmpty() && destination.isNotEmpty() && date.isNotEmpty() && time.isNotEmpty()) {
                ...
                saveTripDetails(Id, origin, destination, "$date $time")
            } else {
